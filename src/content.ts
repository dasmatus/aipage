...
import { initChartReplacer } from "./content-scripts/chart-replacer";
...
(function () {
  ...
  function sync() {
    ...
    const testMenu = document.querySelector(
      ".etest-player-header-inner .etest-header-nav:last-child",
    ) as HTMLElement;
  ...
  async function init() {
    ...
    applyGlobalOverrides(
      result.ai_sidebar_theme || "default",
      globalThemeEnabled,
    );

    // Replace EduPage's performance chart with lightweight-charts
    initChartReplacer();
    ...
    browser.storage.onChanged.addListener((changes) => {
      if (changes.ai_sidebar_theme || changes.ai_sidebar_global) {
        ...
        getStorage(["ai_sidebar_theme", "ai_sidebar_global"]).then((res) => {
          applyGlobalOverrides(
            res.ai_sidebar_theme || "default",
            !!res.ai_sidebar_global,
          );
        });
      }
    });
  ...
  browser.runtime.onMessage.addListener(
    (msg: any, sender: any): Promise<any> | void => {
      ...
      if (msg.action === "get_exam_question") {
        ...
        if (!container || !container.offsetParent)
          return Promise.resolve({ ok: false, error: "No exam content found" });
        ...
        return Promise.resolve({
          ...
          questionText,
          inputType:
            radioInputs.length > 0 ? "choice" : textInput ? "text" : "unknown",
          choices,
        });
      }
      ...
      if (msg.action === "fill_answer") {
        ...
        if (inputType === "choice") {
          ...
          const input = document.querySelector(
            '.etest-player-content input[type="text"], .etest-player-content textarea',
          ) as HTMLInputElement | null;
          if (input) {
            input.value = value;
      ...
      if (msg.action === "get_page_content") {
        return (async () => {
          try {
            ...
            const testContent = document.querySelector(
              ".etest-player-content",
            ) as HTMLElement;
            let text = "";
